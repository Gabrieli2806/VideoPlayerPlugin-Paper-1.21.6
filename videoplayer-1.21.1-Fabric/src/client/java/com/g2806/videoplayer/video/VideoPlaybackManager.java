...
public final class VideoPlaybackManager {
    ...
    private volatile String displayMode = "hud"; // "hud" | "block"
    ...
    public synchronized void play(String id, String source, String mode) {
        this.displayMode = (mode != null && mode.equals("block")) ? "block" : "hud";
        play0(id, source);
    }

    /** Backward-compatible overload (defaults to HUD). */
    public synchronized void play(String id, String source) {
        this.displayMode = "hud";
        play0(id, source);
    }

    private synchronized void play0(String id, String source) {
        stop();
        currentVideoId = id;
        statusText = "Loading...";
        running.set(true);
    ...
    public boolean isHudMode()         { return "hud".equals(displayMode); }
    public boolean isBlockMode()       { return "block".equals(displayMode); }
    public ResourceLocation getTextureId() { return textureId; }
    public int getVideoWidth()  { return videoWidth; }
    public int getVideoHeight() { return videoHeight; }

    /* ======================= frame tick (render thread ~60fps) ======================= */

    /**
     * Consume frames from the queue and upload the current frame to the GPU texture.
     * Called every render tick regardless of display mode so block mode also advances.
     */
    ...
    /**
     * Blit the video full-screen (HUD mode only). Call after tickFrames().
     */
    public void blitFullScreen(GuiGraphics context) {
    ...
    private static String findExecutable(String... names) {
        // 1. Try bare command name (relies on PATH)
        for (String cmd : names) {
            if (tryRun(cmd)) return cmd;
        }
        // 2. Try refreshed PATH from environment (process may have stale PATH)
        String machinePath = System.getenv("PATH");
        if (machinePath != null) {
            for (String dir : machinePath.split(java.util.regex.Pattern.quote(java.io.File.pathSeparator))) {
                for (String name : names) {
                    Path candidate = Path.of(dir, name);
                    if (Files.isExecutable(candidate) && tryRun(candidate.toString())) {
                        return candidate.toString();
                    }
                }
            }
        }
        // 3. Try known common install paths on Windows
        String userHome = System.getProperty("user.home");
        java.util.List<String> knownDirs = java.util.List.of(
            userHome + "\\AppData\\Local\\Microsoft\\WinGet\\Links",
            userHome + "\\AppData\\Local\\Microsoft\\WinGet\\Packages\\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\\ffmpeg-full_build\\bin",
            "C:\\ffmpeg\\bin",
        ...
        for (String dir : knownDirs) {
            for (String name : names) {
                Path candidate = Path.of(dir, name);
                if (Files.exists(candidate) && tryRun(candidate.toString())) {
                    VideoPlayer.LOGGER.info("Found {} at: {}", name, candidate);
                    return candidate.toString();
                }
            }
        }
    ...
    private static boolean tryRun(String cmd) {
        try {
